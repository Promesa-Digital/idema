      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {footerLinks.social.map(social => {
                ...
                return (
                  <a
                    ...
                    className="w-10 h-10 rounded-full bg-white/10 hover:bg-[#0dcaf0] flex items-center justify-center transition-colors"
                  >
                ...
                <li key={link.href}>
                  {link.external ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-white/80 hover:text-[#0dcaf0] transition-colors text-sm"
                    >
                      {link.label}
                    </a>
                  ) : (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {footerLinks.contact.addresses.map((addr, idx) => (
              <div key={idx} className="flex gap-3">
                <FaMapMarkerAlt className="w-5 h-5 text-[#0dcaf0] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-semibold text-[#0dcaf0]">{addr.label}</p>
          ...
          <div className="mt-6 flex flex-col md:flex-row gap-6">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[#0dcaf0]">Email</p>
                <a href={`mailto:${footerLinks.contact.email}`} className="text-white/80 hover:text-[#0dcaf0] text-sm transition-colors">
        ...
        >
          ...
          <div className="flex flex-wrap gap-4 md:gap-6 text-white/70 text-sm">
            <Link to="/politica-privacidad" className="hover:text-[#0dcaf0] transition-colors">
              Políticas de Privacidad
            </Link>
            <Link to="/terminos-y-condiciones" className="hover:text-[#0dcaf0] transition-colors">
              Términos y Condiciones
            </Link>
            <Link to="/libro-reclamaciones" className="hover:text-[#0dcaf0] transition-colors">
              Libro de Reclamaciones
            </Link>
            <Link to="/eliminar-cuenta" className="hover:text-[#0dcaf0] transition-colors">
              Eliminar Cuenta
            </Link>
          </div>
        </motion.div>
